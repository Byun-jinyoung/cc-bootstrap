---
description: PDF 논문을 분석하고 Codex/Antigravity와 교차 검증하여 Obsidian 노트를 생성한다.
---

# 논문 분석 및 교차 검증

사용자가 제공한 PDF 논문을 분석하고, Codex + Antigravity와 교차 검증하여 최종 노트를 생성하라.

> **Migration note (2026-06-18)**: Gemini CLI 지원 종료로 3rd peer 가 Gemini → Antigravity (`agy`) 로 교체됨. MCP 호출도 `mcp__gemini-mcp__ask_gemini` → `mcp__antigravity-mcp__ask_antigravity` 로 변경.
...
└── {논문대표이름}_text.txt    ← 추출 텍스트 (Codex/Antigravity용)
```
...
## 2단계: 3자 독립 분석 (Claude + Codex + Antigravity 병렬)
...
### 2c: Antigravity 독립 분석

mcp__antigravity-mcp__ask_antigravity를 호출. Antigravity 에 paper-analyzer skill 이 설치되어 있어야 한다 (`agy plugin import gemini-cli` 로 직전 gemini-cli 의 paper-analyzer skill 을 상속하거나, 직접 설치).
Antigravity 는 workspace 내 파일만 읽을 수 있으므로, text_path 는 반드시 PROject/Paper/ 아래 경로를 사용.

```
mcp__antigravity-mcp__ask_antigravity 호출:
  prompt: "paper-analyzer skill을 사용하여 논문을 분석하라.
    텍스트 파일: {text_path} (workspace 내 경로)
    전체 분석을 수행하라.
    결과를 PROject/Paper/{논문대표이름}/antigravity_analysis.md로 저장하라."
...
Claude 분석(2a), Codex 분석(2b), Antigravity 분석(2c)을 비교하라:

1. **사실 불일치**: 같은 데이터에 대해 다른 수치/해석을 제시한 부분
2. **누락 차이**: 한쪽만 언급한 핵심 결과나 한계점
3. **해석 차이**: 동일 데이터에 대한 다른 추론 (어느 쪽이 더 근거가 강한지 판단)
4. **Figure 의존성**: Codex가 Figure를 보고 추가로 파악한 내용 (Antigravity는 Figure 미참조)
...
```markdown
## 교차 검증 결과
- 검증 방법: Claude (모델명) + Codex (모델명) + Antigravity (default) 독립 분석 → 3자 비교
- 사실 불일치: (있으면 기술)
