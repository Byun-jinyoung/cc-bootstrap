...
PDF 경로가 주어지면, 아래 Python 코드로 텍스트와 Figure를 직접 추출하라.
...
for i, img in enumerate(images):
    img.save(f"/tmp/paper_page_{i+1}.png")
```
- Figure가 있는 페이지를 이미지로 변환하여 시각적으로 분석

### 사용 기준
- 본문 텍스트 → pdfplumber
- Figure/수식 확인 → pdf2image로 이미지 변환 후 확인
...
3. Figure 이미지를 분석하여 그래프/표/다이어그램 해석
